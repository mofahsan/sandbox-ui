...
function NewRequset() {
  ...
  const [selectedOptionCall, setSelectedOptionCall] =
    useState("Select Endpoint");
  ...
  const handleSend = async () => {
    try {
      ...
      if (
        !validateJson(editorData.Summary || !validateJson(editorData.Header))
      ) {
      ...
      const response = await axios1.post(
        `${env.sandBox}${selectedOptionCall}`,
        editorData.Summary,
        header
      );
  ...
  const generateHeader = async () => {
    const response = await axios.post(
      env.sandBox + "/createHeader",
      editorData.Summary
    );
    if (response) {
      editorData.Header = JSON.stringify({
